    ...
    try {
        ...
        if (!await fs.access(PRODUCTS_FILE).then(()=>true).catch(()=>false)) {
            await fs.writeFile(PRODUCTS_FILE, JSON.stringify([
                ...
                { id: 3, name: 'FPV Drone iFlight Titan', category: 'Дрони', price: 32000, description: 'FPV дрон 6S', variants: ['5"', '6"', '7"'], image: null, createdAt: new Date().toISOString() }
            ], null, 2));
...
app.post('/api/logo', isAdmin, upload.single('logo'), async (req, res) => {
    ...
    await fs.writeFile(LOGO_FILE, path);
    res.json({ path });
});
...
app.post('/api/banner', isAdmin, upload.single('banner'), async (req, res) => {
    if (!req.file) return res.status(400).json({ error: 'Файл не завантажено' });
    const path = `/uploads/${req.file.filename}`;
    await fs.writeFile(BANNER_FILE, path);
    res.json({ path });
});

...
// Функція ініціалізації даних з try/catch
async function initData() {
    try {
        ...
        // Категорії
        try {
            await fs.readFile(CATEGORIES_FILE, 'utf8');
        } catch {
            await fs.writeFile(CATEGORIES_FILE, JSON.stringify(['Дрони', 'Реб'], null, 2));
            console.log('✅ Створено categories.json');
        }

        // Товари
        try {
            await fs.readFile(PRODUCTS_FILE, 'utf8');
        } catch {
            await fs.writeFile(PRODUCTS_FILE, JSON.stringify([
                { id: 1, name: 'Квадрокоптер DJI Mavic 3', category: 'Дрони', price: 85000, description: 'Професійний дрон', variants: ['Standard', 'Pro', 'Cine'], image: null, createdAt: new Date().toISOString() },
                { id: 2, name: 'Карбонові реб 3K', category: 'Реб', price: 1200, description: 'Карбонові ребра', variants: ['200mm', '250mm', '300mm'], image: null, createdAt: new Date().toISOString() }
            ], null, 2));
            console.log('✅ Створено products.json');
        }

        // Про нас
        try {
            await fs.readFile(ABOUT_FILE, 'utf8');
        } catch {
            await fs.writeFile(ABOUT_FILE, 'Ми — команда професіоналів, яка займається продажем дронів та реб. Працюємо з 2020 року.');
            console.log('✅ Створено about.txt');
        }

        // Контакти
        try {
            await fs.readFile(CONTACT_FILE, 'utf8');
        } catch {
            ...
            console.log('✅ Створено contact.txt');
        }

        // Замовлення
        try {
            await fs.readFile(ORDERS_FILE, 'utf8');
        } catch {
            await fs.writeFile(ORDERS_FILE, JSON.stringify([], null, 2));
            console.log('✅ Створено orders.json');
        }

        // Соцмережі
        try {
            await fs.readFile(SOCIAL_FILE, 'utf8');
        } catch {
            ...
            console.log('✅ Створено social.json');
        }

        // Логотип, банер, бренд-лого
        try { await fs.writeFile(LOGO_FILE, ''); } catch(e) {}
        try { await fs.writeFile(BANNER_FILE, ''); } catch(e) {}
        try { await fs.writeFile(BRAND_LOGO_FILE, ''); } catch(e) {}

        console.log('✅ Ініціалізацію даних завершено');
    } catch (err) {
        console.error('❌ Помилка ініціалізації:', err);
        // Не викидаємо помилку, щоб сервер запустився навіть якщо щось пішло не так
    }
}

// Запускаємо ініціалізацію, але не чекаємо на неї
initData();
...
app.get('/api/banner', async (req, res) => {
    try {
        const p = await fs.readFile(BANNER_FILE, 'utf8');
        res.send(p || '');
    } catch(e) { res.send(''); }
});
...
app.get('/api/brand-logo', async (req, res) => {
    try {
        const p = await fs.readFile(BRAND_LOGO_FILE, 'utf8');
        res.send(p || '');
    } catch(e) { res.send(''); }
});
...
app.get('/api/social', async (req, res) => {
    try {
        ...
        if (!req.session.user?.isAdmin) {
            res.json(s.filter(si => si.active));
        } else {
            res.json(s);
        }
    } catch(e) { res.json([]); }
...
app.get('/api/stats', isAdmin, async (req, res) => {
    try {
        ...
        res.json({ productsCount: 0, categoriesCount: 0, ordersCount: 0, totalOrdersSum: 0, variantsCount: 0 });
    }
});

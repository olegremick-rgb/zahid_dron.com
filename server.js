...
async function loadDatabase() {
    try {
        ...
        db.about = await readText('about.txt', 'Ми — команда професіоналів, яка займається продажем дронів та комплектуючих.');
...
async function writeJSON(filename, data) {
    try {
        ...
        console.error(`Помилка запису ${filename}:`, err);
    }
}
...
app.post('/api/admin/change-password', isAdmin, async (req, res) => {
    ...
    if (user.password !== currentPassword) return res.status(401).json({ error: 'Невірний пароль' });
